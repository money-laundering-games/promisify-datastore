		...
		const DataStoreAsync = {
			GetVersionAsync(
				...
				return Promise.promisify(() => {
					return internal_store.GetVersionAsync(key, version);
				})();
			},
			ListKeysAsync(
				prefix?: string,
				...
				return Promise.promisify(() => {
					return internal_store.ListKeysAsync(prefix, pageSize, cursor, excludeDeleted);
				})();
			},
			ListVersionsAsync(
				key: string,
				...
				return Promise.promisify(() => {
					return internal_store.ListVersionsAsync(key, sortDirection, minDate, maxDate, pageSize);
				})();
			},
			RemoveVersionAsync(key: string, version: string): Promise<void> {
				return Promise.promisify(() => {
					return internal_store.RemoveVersionAsync(key, version);
				})();
			},
			OnUpdate(key: string, callback: Callback): RBXScriptConnection {
				return internal_store.OnUpdate(key, callback);
			},
			GetAsync(key: string): Promise<LuaTuple<[unknown, DataStoreKeyInfo]>> {
				return Promise.promisify(() => {
					return internal_store.GetAsync(key);
				})();
			},
			IncrementAsync(
				key: string,
				...
				return Promise.promisify(() => {
					return internal_store.IncrementAsync(key, delta, userIds, options);
				})();
			},
			RemoveAsync(key: string): Promise<LuaTuple<[unknown, DataStoreKeyInfo | undefined]>> {
				return Promise.promisify(() => {
					return internal_store.RemoveAsync(key);
				})();
			},
			SetAsync(key: string, value?: unknown, userIds?: number[], options?: DataStoreSetOptions): Promise<string> {
				return Promise.promisify(() => {
					return internal_store.SetAsync(key, value, userIds, options);
				})();
			},
			UpdateAsync(
				key: string,
				...
				return Promise.promisify(() => {
					return internal_store.UpdateAsync(key, transformFunction);
				})();
			},
		};
